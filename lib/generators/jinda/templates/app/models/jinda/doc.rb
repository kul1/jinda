...
module Jinda
  class Doc
    ...
    def self.search(q, page, per_page = PER_PAGE)
      paginate per_page: per_page, page: page, conditions:         ['content_type=? AND data_text LIKE ? AND (ma_secured=? OR ma_user_id=?)',
    ...
    def self.search_ma_secured(q, page, per_page = PER_PAGE)
      paginate per_page: per_page, page: page, conditions:         ['content_type=? AND data_text LIKE ?', 'output', "%#{q}%"],
               order: 'ma_xmain_id DESC', select: 'DISTINCT ma_xmain_id'
    end
  end
end
