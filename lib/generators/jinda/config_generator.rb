  ...
  module Generators
    class ConfigGenerator < Rails::Generators::Base
      ...
      def setup_docker
        ...
        FileUtils.cp "#{source_paths[0]}/docker-compose-web.yml", "docker-compose-web.yml"
        FileUtils.cp "#{source_paths[0]}/entrypoint.sh", "entrypoint.sh"
        FileUtils.cp "#{source_paths[0]}/DOCKER.md", "DOCKER.md"
      end
      ...
      def finish
        ...
        Rails.logger.debug "-----------------------------------------\n"
        Rails.logger.debug "      Docker options (MongoDB on standard port 27017):"
        Rails.logger.debug "-----------------------------------------\n"
        Rails.logger.debug "MongoDB only (recommended for development):\n"
        Rails.logger.debug "  docker compose -f docker-compose-mongodb.yml up -d\n"
        Rails.logger.debug "\n"
        Rails.logger.debug "Full stack with web container:\n"
        Rails.logger.debug "  docker compose -f docker-compose-web.yml up -d\n"
        Rails.logger.debug "  Note: Edit docker-compose-web.yml for local gem paths\n"
        Rails.logger.debug "\n"
        Rails.logger.debug "Simple MongoDB only:\n"
        Rails.logger.debug "  docker compose -f docker-compose.yml up -d\n"
        Rails.logger.debug "-----------------------------------------\n"
